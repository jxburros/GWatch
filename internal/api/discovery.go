...
func (s *Server) handleAddFromDiscovery(w http.ResponseWriter, r *http.Request) {
	...
	// A node can be in several groups, and the picker may name one the old
	// way or several the new way; the template's own group is replaced, not
	// added to, so a sweep lands where the person said and nowhere else.
	groups := model.NormalizeGroups(append(append([]string(nil), req.Groups...), req.Group))
	...
	for _, item := range req.Items {
		...
		n := nodeFromTemplate(tmpl, ip, name, groups)
...
// typed in by hand.
func nodeFromTemplate(t model.NodeTemplate, ip, name string, groups []string) model.Node {
	n := t.Node
	n.ID = 0
	n.Host = ip
	n.Name = name
	...
	if len(groups) > 0 {
		...
		n.Groups = append([]string(nil), t.Node.Groups...)
	}
	n.SyncGroups()
