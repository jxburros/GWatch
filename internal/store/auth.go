...
func (s *Store) CreateUser(ctx context.Context, username, passwordHash string, role auth.Role) (model.User, error) {
	...
	res, err := s.Exec(ctx, `INSERT INTO users(username, password_hash, role, created_at, updated_at) VALUES (?,?,?,?,?)`,
...
func (s *Store) GetUser(ctx context.Context, id int64) (model.User, error) {
	u, err := scanUser(s.reader.QueryRowContext(ctx, "SELECT "+userCols+" FROM users WHERE id = ?", id))
...
func (s *Store) GetUserByName(ctx context.Context, username string) (model.User, string, error) {
	row := s.reader.QueryRowContext(ctx, "SELECT "+userCols+", password_hash FROM users WHERE username = ? COLLATE NOCASE", strings.TrimSpace(username))
	var u model.User
...
func (s *Store) ListUsers(ctx context.Context) ([]model.User, error) {
	rows, err := s.reader.QueryContext(ctx, "SELECT "+userCols+" FROM users ORDER BY username COLLATE NOCASE")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CountAdmins(ctx context.Context) (int, error) {
	var n int
	err := s.reader.QueryRowContext(ctx, "SELECT COUNT(*) FROM users WHERE role = ?", string(auth.RoleAdmin)).Scan(&n)
	return n, err
}
...
func (s *Store) GetSession(ctx context.Context, tokenHash string, extend time.Duration) (model.User, error) {
	row := s.reader.QueryRowContext(ctx, "SELECT user_id, expires_at FROM sessions WHERE token_hash = ?", tokenHash)
	var userID int64
	var expires string
...
func (s *Store) CountSessions(ctx context.Context) (int, error) {
	var n int
	err := s.reader.QueryRowContext(ctx, "SELECT COUNT(*) FROM sessions").Scan(&n)
	return n, err
}
...
func (s *Store) CreateAPIKey(ctx context.Context, name, prefix, keyHash string, scope auth.Scope, createdBy string) (model.APIKey, error) {
	...
	res, err := s.Exec(ctx, `INSERT INTO api_keys(name, prefix, key_hash, scope, created_by, created_at) VALUES (?,?,?,?,?,?)`,
...
func (s *Store) GetAPIKey(ctx context.Context, id int64) (model.APIKey, error) {
	k, err := scanAPIKey(s.reader.QueryRowContext(ctx, "SELECT "+apiKeyCols+" FROM api_keys WHERE id = ?", id))
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) ListAPIKeys(ctx context.Context) ([]model.APIKey, error) {
	rows, err := s.reader.QueryContext(ctx, "SELECT "+apiKeyCols+" FROM api_keys ORDER BY revoked_at IS NOT NULL, id DESC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) LookupAPIKey(ctx context.Context, keyHash string) (model.APIKey, error) {
	k, err := scanAPIKey(s.reader.QueryRowContext(ctx, "SELECT "+apiKeyCols+" FROM api_keys WHERE key_hash = ?", keyHash))
	if errors.Is(err, sql.ErrNoRows) {
		return k, ErrNotFound
	}
	if err != nil {
