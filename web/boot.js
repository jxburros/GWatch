...
// parser stops here until it has run, so the remembered theme, accent and
// sidebar state are on <html> before anything is drawn and nothing flashes.
// It lives in its own file rather than inline so the Content-Security-Policy
// can stay at 'self' with no hash to keep in step with this code. Settings
// from the service override all of it once the app has loaded.
(function () {
  try {
