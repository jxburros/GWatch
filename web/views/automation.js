...
export function actionEditor(action = {}, { nodes = [], defaultInterpreter = 'sh', nodeId = null } = {}) {
  const a = { type: 'http', method: '', url: '', headers: {}, body: '', ignoreTlsErrors: false, expectedStatus: '', repo: '', gitArgs: '', interpreter: defaultInterpreter, command: '', code: '', workDir: '', nodeId: nodeId, timeoutSeconds: 30, ...(action || {}) };
  ...
  // script
  const interp = selectInput({ options: INTERPRETERS, value: a.interpreter || defaultInterpreter, onchange: () => { a.interpreter = interp.value; cmdField.hidden = interp.value !== 'custom'; } });
  ...
  function renderType() {
    ...
    switch (a.type) {
      ...
      case 'git':
        ...
        break;
      case 'script':
        typeArea.append(h('div', { class: 'form-grid' }, field({ label: 'Interpreter', input: interp }), field({ label: 'Working directory', input: workDir }), h('div', { class: 'span-2' }, cmdField), field({ label: 'Code', input: code, cls: 'span-2' })));
        break;
      case 'run_node':
  ...
  Object.defineProperty(wrap, 'value', { get: () => {
    ...
    if (a.type === 'script') Object.assign(out, { interpreter: a.interpreter || defaultInterpreter, command: a.command || '', code: a.code || '', workDir: a.workDir || '' });
...
export function openEndpointEditor(existing, { nodes = [] } = {}) {
  return new Promise(async (resolve) => {
    const meta = await automationMeta();
    const e = existing ? { ...existing, action: { ...(existing.action || {}) } } : { name: '', slug: '', description: '', enabled: true, method: 'ANY', token: '', action: { type: 'run_node' } };
    let result = null;
    ...
    const token = textInput({ value: e.token || '', class: 'mono', placeholder: 'Optional shared secret', autocomplete: 'off' });
    const genBtn = h('button', { class: 'btn btn-sm', type: 'button', onclick: () => { token.value = randomToken(); } }, icon('refresh'), 'Generate');
    ...
    async function submit() {
      ...
      const payload = { ...e, name: name.value.trim(), slug: slug.value.trim() || slugify(name.value), description: desc.value.trim(), method: method.value, token: token.value.trim(), enabled: enabled.input.checked, action: editor.value };
      try {
...
export function endpointRow(e, { nodes = [], onChange } = {}) {
  ...
  return h('div', { class: 'endpoint-row' },
    h('div', { style: { minWidth: 0 } },
      h('div', { class: 't-name', style: { fontWeight: 650, display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' } }, e.name, actionBadge(e.action), h('span', { class: 'tag' }, e.method || 'ANY'), e.token ? h('span', { class: 'tag', title: 'Token required' }, icon('lock'), ' token') : null, e.enabled === false ? h('span', { class: 'tag' }, 'disabled') : null),
