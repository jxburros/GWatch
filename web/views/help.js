...
const TOPICS = [
  ...
  },
  {
    ...
    icon: 'server',
    q: 'ping tcp http https dns certificate keyword json custom script hardware snmp oid router switch template group tag dependency importance interval timeout',
    body: () => [
  ...
  },
  {
    ...
    body: () => [
      ...
      h('p', null, h('b', null, 'Ping fails but the host is up.'), ' ICMP needs a raw socket on Windows, which the service account has. On Linux GWatch tries unprivileged ping sockets, then a raw socket, then the system ', h('code', null, 'ping'), ' command.'),
